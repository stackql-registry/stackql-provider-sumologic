...
<tr>
    ...
    <td><a href="#parameter-id"><code>id</code></a>, <a href="#parameter-region"><code>region</code></a></td>
    <td></td>
    <td>Generates a new secret for the access key that is passed in the call, keeping the same access ID.</td>
</tr>
</tbody>
</table>
...
<tr id="parameter-id">
    <td><CopyableCode code="id" /></td>
    <td><code>string</code></td>
    <td>The accessId of the access key to rotate the secret for.</td>
</tr>
...
```sql
UPDATE sumologic.access_keys.access_keys
SET 
disabled = {{ disabled }},
...
Generates a new secret for the access key that is passed in the call, keeping the same access ID.

```sql
EXEC sumologic.access_keys.access_keys.rotate_secret 
@id='{{ id }}' --required, 
