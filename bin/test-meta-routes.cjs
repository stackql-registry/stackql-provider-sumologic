...
for (let i = 0; i < args.length; i++) {
  if (args[i].startsWith('--')) {
    switch (args[i]) {
      ...
      case '--help':
        console.log(`
Usage: test-meta-routes.cjs <provider> [OPTIONS]
...
if (!provider) {
  ...
  console.error('Usage: test-meta-routes.cjs <provider> [OPTIONS]');
  process.exit(1);
}
...
// resources whose select response is a scalar body (a bare JSON string or
// an XML document) - a single anonymous column at query time, so DESCRIBE
// EXTENDED is legitimately empty:
//   (account.account_owner is wrapped into an object by post_process.mjs and has a column)
//   saml.identity_provider_metadata  GET .../identityProviders/{id}/metadata returns application/xml
const SCALAR_RESPONSE_RESOURCES = new Set(['identity_provider_metadata']);
